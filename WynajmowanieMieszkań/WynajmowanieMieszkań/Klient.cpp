...
Klient::Klient(string imie, string nazwisko, int id, string ulica, string miasto, string kodPocztowy, int wiek) 
	: Osoba(imie, nazwisko, id), ulica(ulica), miasto(miasto), kodPocztowy(kodPocztowy), wiek(wiek)
{
	const std::regex base_regex("^[0-9]{2}-[0-9]{3}$");
	std::smatch base_match;
	if (!std::regex_match(kodPocztowy, base_match, base_regex)) 
	{
		cout << "Niepoprawny kod pocztowy!" << endl;
		this->kodPocztowy = "00-000";
	}
}
